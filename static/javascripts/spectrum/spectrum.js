...
(function (window, $, undefined) {
    var defaultOpts = {
        ...
        className: "",
        theme: "sp-light",
        palette: ['fff', '000', 'invisible'],
        non_code_color: '',
        non_code_colors: { 'invisible': 'fff' },
        selectionPalette: []
    },
    spectrums = [],
    IE = $.browser.msie,
    ...
    paletteTemplate = function (p, color, className, non_code_colors, cur_non_code_col) {
        var html = [];
        for (var i = 0; i < p.length; i++) {
            if( non_code_colors.has( p[i] ) ) {
                var tiny = tinycolor( non_code_colors[p[i]] );
                var c = tiny.toHsl().l < .5 ? "sp-thumb-dark" : "sp-thumb-light";
                if( cur_non_code_col == p[i] ) c += " sp-thumb-active";
                ...
                var c = tiny.toHsl().l < .5 ? "sp-thumb-dark" : "sp-thumb-light";
                c += (cur_non_code_col == '' && tinycolor.equals(color, p[i])) ? " sp-thumb-active" : "";
                html.push('<span title="'
                    + tiny.toHexString()
                    + '" data-color="'
                    + tiny.toHexString()
    ...
    function spectrum(element, o) {
        ...
        var opts = instanceOptions(o, element),
            ...
            non_code_color = '',
            non_code_colors = opts.non_code_colors,
        ...
        non_code_colors.has = function( key ) {
          for( var k in this ) {
        ...
        function drawPalette() {
            ...
            var html = $.map(paletteArray, function (palette, i) {
                return paletteTemplate(palette, currentColor, "sp-palette-row sp-palette-row-" + i, non_code_colors, non_code_color);
            });

            if (selectionPalette) {
                html.push(paletteTemplate(getUniqueSelectionPalette(), currentColor, "sp-palette-row sp-palette-row-selection", non_code_colors, non_code_color));
            }
        ...
        function drawInitial() {
            if (showInitial) {
                ...
                var current = get();
                initialColorContainer.html(paletteTemplate([initial, current], current, "sp-palette-row-initial", non_code_colors, non_code_color));
            }
        }
        function dragStart() {
        ...
        function setFromTextInput() {
            ...
            if (tinycolor( textInput.val() ).ok || non_code_colors.has( textInput.val() )) {
                set(textInput.val());
            }
            else {
        ...
        function set(color, ignoreFormatChange) {
            ...
            if( non_code_colors.has( color ) && non_code_color == color ) {
                return;
            } else if( !non_code_colors.has( color ) && tinycolor.equals(color, get())) {
                return;
            }
            ...
            if( non_code_colors.has( color ) ) {
              non_code_color = color;
              newColor = tinycolor( non_code_colors[non_code_color] );
            } else {
              non_code_color = ''
        ...
        function updateOriginalInput(fireCallback) {
            ...
            if (isInput) {
                if( non_code_color != '' ) {
                  boundElement.val(non_code_color).change();
                } else {
